/**
 * @file    system.c
 * @note    KSCOS 系统 app — PC (时间/内存服务)
 *
 * 固定地址 app, 提供时间/内存/初始化等内核服务。
 * PC 无芯片初始化 (Sleep/GetTickCount 为系统 API)。
 */
...
/* STM32 有 SysTick 递增; PC 占位 (GetTickCount 为准) */
...
int system_app_cmd(app_t* app, const char* cmdname, const char** argv)
{
    ...
    if (strcmp(cmdname, SYS_CMD_TIME) == 0) {
        if (app->output_data) *(uint32_t*)app->output_data = GetTickCount();
        return 1;
    }
    if (strcmp(cmdname, SYS_CMD_DELAY) == 0) {
        if (!APPCMD_HAS(argv, 't')) return -1;
        Sleep((DWORD)strtoul(argv[APPCMD_ARG('t')], NULL, 0));
        return 1;
    }
