/**
 * @file    w25qxx_base.c
 * @note    W25Q64 SPI NOR Flash 基础驱动 (Linux BSP, 文件模拟)
 *
 * ============================================================
 * 基本信息
 * ============================================================
 ...
 *
 * ============================================================
 * 与 bsp/pc/w25qxx_base.c 的差异
 * ============================================================
 *   - GetModuleFileNameA + '\\' → ksc_data_path() (见 ksc_linux_path.h)
 *   - CreateDirectoryA         → mkdir() (经 ksc_data_mkdir)
 *   - MAX_PATH                 → KSC_PATH_MAX (512)
 */
...
#define PC_FLASH_PATH_MAX  128

typedef struct {
    FILE*  fp;
    uint32_t addr;
    char   path[PC_FLASH_PATH_MAX];
} pc_flash_ctx_t;

static void pc_flash_path(char* path, size_t sz)
{
    if (ksc_data_path(path, sz, "flash.bin") != 0)
        snprintf(path, sz, ".data/flash.bin");   /* 兜底 */
}
...
static int pc_flash_open(app_t* app)
{
    ...
    /* 只创建 .data/ 目录本身; 已存在则不动作, 绝不删除/清空已有 flash.bin */
    ksc_data_mkdir(ctx->path);
...
static int pc_flash_write(app_t* app, void* data, uint32_t count, uint32_t mode)
{
    ...
    case 3:
        ...
        fseek(ctx->fp, (long)ctx->addr, SEEK_SET);
...
/* Linux appcmd — 与 STM32 w25_cmd / PC 命令集对齐。
...
REGISTER_APP_EX("w25qxx_base", "0", "0", &pc_flash_ops,
    "Linux file-backed flash emulation");
